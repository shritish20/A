...
class AutoTradingEngine:
    ...
    def execute_mandate(self, analysis_data: Dict, mandate: TradingMandate) -> bool:
        ...
        with self._lock:
            ...
            # Entry window guard — no trades before 9:30 AM IST (opening noise)
            ...
            with self.db_session_factory() as db:
                result = self.system.execute_strategy(strategy, db, external_metrics=analysis_data.get('external_metrics'))
...
class JSONCacheManager:
    ...
    def get_external_metrics(self) -> ExternalMetrics:
        ...
        # Full flow regime logic — matching Colab v33
        flow_regime = "NEUTRAL"
        if fii_data and fii_data.get("FII"):
            fii_obj = fii_data["FII"]
            fut_bullish = fii_obj.fut_net > 0
            # option bias: net call buying vs net put buying
...
# ============================================================================
# PARTICIPANT DATA FETCHER  
# Uses NSE CSV Archives (public static files — no cookies, works on AWS)
# Same approach as Colab v33 which works reliably
# ============================================================================
...
class UpstoxOrderExecutor:
        ...
        for leg in strategy.legs:
            ...
            try:
                ...
                if sl_response.status == "success" and sl_response.data:
                    # SDK returns gtt_order_ids (array)
    ...
    def exit_position(self, trade: TradeJournal, exit_reason: str, 
        ...
        # Close shorts first (remove naked risk), then sell wings
        ...
        for leg in legs_sorted:
            try:
                ...
                # Use LIMIT at current LTP to avoid MARKET slippage on OTM options
...
class RegimeEngine:
    ...
    def calculate_scores(self, vol_metrics: VolMetrics, struct_metrics: StructMetrics,
        ...
        # Score stability — how robust the composite is across different weight schemes
        # Matches Colab v33 logic: low variance = stable signal
...
class StrategyFactory:
    ...
    def construct_iron_fly(self, expiry_date: date, allocation: float) -> Optional[ConstructedStrategy]:
        try:
            ...
            # Lot sizing by margin (correct) — fallback to premium if API fails
    ...
    async def start(self):
        ...
        while self._running:
            try:
                ...
                # Skip analytics on weekends/holidays — save API quota
...
class VolGuardSystem:
    def __init__(self):
        ...
        self.logger.info("VolGuard System v4.0 initialized")
    ...
    def execute_strategy(self, strategy: ConstructedStrategy, db: Session, external_metrics=None) -> Dict:
        if not strategy.validation_passed:
            return {
                "success": False,
        ...
        if result['success']:
            ...
            # Populate associated_event_date from nearest veto event so pre-event
            # square-off logic in should_square_off_position() fires correctly
...
async def lifespan(app: FastAPI):
    logger.info("=" * 70)
    logger.info("VolGuard v4.0 Starting...")
    logger.info("=" * 70)
    ...
    logger.info("VolGuard v4.0 shutting down...")
    if alert_service:
...
app = FastAPI(
    title="VolGuard v4.0 - Production",
...
def get_professional_dashboard(
    ...
    try:
        ...
        # Mirrors Colab v33 display: full table with fut_long, fut_short, call_net, put_net, stock_net
...
def get_live_positions(
    ...
    try:
        ...
        # Live greeks from SDK — replaces stale entry_greeks_snapshot
        live_greeks_map = {}
        try:
        ...
        for trade in active_trades:
            ...
            for leg in legs_data:
                ...
                # Use live greeks if available, fallback to entry snapshot
        ...
        # Theta/Vega ratio — with zero guard
        ...
        # ATM straddle price + expected move range for the primary active trade
        straddle_info = None
        if active_trades:
            try:
                ...
                spot = volguard_system.fetcher.get_ltp_with_fallback(
                    "NSE_INDEX|Nifty 50"
                ) or 0
