        ...
        for leg in strategy.legs:
            ...
            try:
                ...
                if sl_response.status == "success" and sl_response.data:
                    # SDK returns gtt_order_id (singular), not gtt_order_ids
                    raw_sl_id = getattr(sl_response.data, 'gtt_order_id', None)
                    sl_ids = [raw_sl_id] if raw_sl_id else []
            ...
            try:
                ...
                if tgt_response.status == "success" and tgt_response.data:
                    # SDK returns gtt_order_id (singular), not gtt_order_ids
                    raw_tgt_id = getattr(tgt_response.data, 'gtt_order_id', None)
                    tgt_ids = [raw_tgt_id] if raw_tgt_id else []
...
class VolGuardSystem:
    def __init__(self):
        ...
        if DynamicConfig.get("AUTO_TRADING") and SystemConfig.UPSTOX_ACCESS_TOKEN:
            ...
            self.executor = UpstoxOrderExecutor(self.fetcher, alert_service=None)  # alert_service wired in lifespan
        else:
...
async def lifespan(app: FastAPI):
    ...
    # Wire alert_service to executor so exit_position can send alerts
...
async def receive_upstox_token(payload: dict):
    ...
    if volguard_system:
        try:
            ...
            fetcher.quote_api      = upstox_client.MarketQuoteApi(api_client)           # was market_quote_api ❌
            fetcher.quote_api_v3   = upstox_client.MarketQuoteV3Api(api_client)         # was MISSING ❌
            fetcher.options_api    = upstox_client.OptionsApi(api_client)               # was option_chain_api ❌
            ...
            fetcher.user_api       = upstox_client.UserApi(api_client)                  # was MISSING ❌
            fetcher.pnl_api        = upstox_client.TradeProfitAndLossApi(api_client)    # was MISSING ❌
            fetcher.market_api     = upstox_client.MarketHolidaysAndTimingsApi(api_client)  # was MarketDataApi ❌
