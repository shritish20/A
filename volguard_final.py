...
class DynamicConfig:
    ...
    def initialize(cls, db_session_factory):
        ...
        try:
            # Ensure Config table exists - SQLAlchemy 2.0 compatible
            db.execute(text("""
    ...
    def _persist(cls, key, value, db=None):
        """Persist to database - SQLAlchemy 2.0 compatible"""
        close_db = False
        if db is None:
...
class EconomicCalendarEngine:
    """Fetch and classify economic events - ALL UNICODE FIXED"""
    ...
    def get_square_off_for_event(self, event_date: datetime) -> Optional[datetime]:
        ...
        # Create square off time at 2:00 PM IST
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for e in data.get('data', []):
                    ...
                    event_type, is_veto, square_off = self.classify_event(title, country, importance_code, ist_time)
...
class UpstoxFetcher:
    ...
    def get_expiries(self) -> Tuple[Optional[date], Optional[date], Optional[date], int, List[date]]:
        ...
        Returns (weekly_exp, monthly_exp, next_weekly_exp, lot_size, all_expiries)
        """
        try:
...
# ============================================================================
# MARKET DATA STREAMER - V3 COMPLIANT - COMPLETE IMPLEMENTATION
# ============================================================================
...
class MarketUpdate:
    ...
    def from_feed(cls, instrument_key: str, feed_data: Any) -> 'MarketUpdate':
        """Convert SDK feed to standardized format"""
        update = cls(instrument_key=instrument_key, ltp=0.0)
        
        # Handle different feed types based on SDK structure
...
class VolGuardMarketStreamer:
    """
    V3.3 SPEC COMPLIANT: Market Data WebSocket Streamer
    - Uses SDK's MarketDataStreamerV3 directly
    - No manual Protobuf handling - SDK does it
    - Automatic reconnection with configurable backoff
    - Thread-safe callback dispatch
    """
    
    # SDK Mode to our internal mapping
    MODE_MAP = {
        "ltpc": "ltpc",
        "full": "full", 
    ...
    def __init__(self, api_client: upstox_client.ApiClient):
        ...
        self._subscribed_instruments: Dict[str, str] = {}  # key -> mode
        self.is_connected = False
        self.logger = logging.getLogger(f"{self.__class__.__name__}")
        
    def on(self, event: str, callback: Callable):
        """Register event handler - SDK compatible"""
        with self._lock:
    ...
    def connect(self, instrument_keys: Optional[List[str]] = None, mode: str = "ltpc"):
        """
        Establish WebSocket connection with initial subscriptions
        Uses SDK's built-in connection management
        """
        try:
            # Map mode to SDK expected values
            sdk_mode = self.MODE_MAP.get(mode, "ltpc")
            
            # Instantiate SDK streamer
            if instrument_keys:
            ...
            # Configure auto-reconnect with optimal settings
            self.streamer.auto_reconnect(True, 5, 10)  # 5 sec interval, 10 retries
    ...
    def subscribe(self, instrument_keys: List[str], mode: str):
        ...
        Thread-safe, tracks subscriptions for reconnection
        """
        with self._lock:
    ...
    def auto_reconnect(self, enable: bool, interval: int = 5, retry_count: int = 10):
        ...
        with self._lock:
            if self.streamer:
                ...
                self.logger.info(f"Auto-reconnect: enabled={enable}, interval={interval}s, retries={retry_count}")
    ...
    def get_ltp(self, instrument_key: str) -> Optional[float]:
        ...
        with self._lock:
            return self._latest_prices.get(instrument_key) if hasattr(self, '_latest_prices') else None
    ...
    def _on_sdk_message(self, message):
        """
        SDK message event - message is ALREADY DECODED by SDK
        No Protobuf handling needed - SDK does it internally
        """
        try:
            if hasattr(message, 'feeds'):
                # Multiple feeds in one message
                for instrument_key, feed_data in message.feeds.items():
                    update = MarketUpdate.from_feed(instrument_key, feed_data)
                    
                    if hasattr(self, '_latest_prices'):
                        with self._lock:
    ...
    def _on_sdk_auto_reconnect_stopped(self):
        """SDK auto-reconnect stopped event"""
        self.logger.error("MarketDataStreamerV3 auto-reconnect stopped - max retries exceeded")
...
# ============================================================================
# PORTFOLIO DATA STREAMER - V3 COMPLIANT - COMPLETE IMPLEMENTATION
# ============================================================================

@dataclass
class PortfolioUpdate:
    """Portfolio update from WebSocket"""
    update_type: str  # order, gtt_order, position, holding
    user_id: str
    ...
    def from_sdk_message(cls, message: Dict) -> 'PortfolioUpdate':
        """Convert SDK portfolio message to standardized format"""
        update = cls(
...
class VolGuardPortfolioStreamer:
    """
    V3.3 SPEC COMPLIANT: Portfolio WebSocket Streamer
    - Uses SDK's PortfolioDataStreamer directly
    - Configurable update types (order, position, holding, gtt)
    ...
    def connect(self, 
        ...
        """
        Establish portfolio WebSocket connection
        Exactly matches SDK constructor parameters
        """
        try:
            # Instantiate SDK PortfolioDataStreamer with exact parameter names
    ...
    def disconnect(self):
        """Gracefully disconnect portfolio WebSocket"""
        with self._lock:
            if self.streamer:
                try:
                    self.streamer.disconnect()
    ...
    def get_order_status(self, order_id: str) -> Optional[Dict]:
        """Get latest order status from WebSocket cache"""
        with self._lock:
    ...
    def _on_sdk_message(self, message):
        """
        SDK message event - message is Dict from SDK
        PortfolioDataStreamer returns Python dict, not protobuf
        """
        try:
...
# ============================================================================
# UPSTOX ORDER EXECUTOR - COMPLETE IMPLEMENTATION
# ============================================================================

class UpstoxOrderExecutor:
    """
    V3.3 SPEC COMPLIANT: Complete order execution
    - OrderApiV3 for HFT endpoint
    - OrderApi for multi-order operations
    - GTT orders with trailing stop loss
    - Complete error handling with retries
    - Algo name header support
    """
    ...
    def place_multi_order(self, strategy: ConstructedStrategy) -> Dict:
        """
        V3.3 SPEC COMPLIANT: Execute complete strategy with multi-order API
        - BUY orders first, then SELL orders
        - Auto-slicing enabled for large quantities
        - Correlation ID tracking for each leg
    ...
    def _place_gtt_stop_losses(self, strategy: ConstructedStrategy, filled_order_ids: List[str]) -> List[str]:
        """
        V3.3 SPEC COMPLIANT: Multi-leg GTT with trailing stop loss
        Uses OrderApiV3 with MULTIPLE type and STOPLOSS strategy
        """
        gtt_ids = []
    ...
    def exit_position(self, trade: TradeJournal, exit_reason: str, 
        ...
        """
        V3.3 SPEC COMPLIANT: Close a position and cancel GTTs
        Places actual MARKET orders to exit
        """
...
class AnalyticsEngine:
    ...
    def get_time_metrics(self, weekly: date, monthly: date, next_weekly: date, all_expiries: List[date]) -> TimeMetrics:
...
class RegimeEngine:
    ...
    def generate_mandate(self, score: RegimeScore, vol_metrics: VolMetrics,
        ...
        if expiry_type == "MONTHLY" and time_metrics.is_expiry_day_monthly:
            veto_reasons.append("EXPIRY_DAY_MONTHLY")
        if expiry_type == "NEXT_WEEKLY" and time_metrics.is_expiry_day_next_weekly:
            veto_reasons.append("EXPIRY_DAY_NEXT_WEEKLY")
    ...
    def _validate_strategy(self, legs: List[OptionLeg]) -> List[str]:
        ...
        for leg in legs:
            if leg.oi < DynamicConfig.MIN_OI:
                errors.append(f"{leg.option_type} {leg.strike} OI {leg.oi} < {DynamicConfig.MIN_OI}")
        ...
        for leg in legs:
            if leg.ask > 0 and leg.bid > 0:
                ...
                if spread_pct > DynamicConfig.MAX_BID_ASK_SPREAD_PCT:
                    errors.append(f"{leg.option_type} {leg.strike} spread {spread_pct:.1f}% > {DynamicConfig.MAX_BID_ASK_SPREAD_PCT}%")
...
class PositionMonitor:
    ...
    async def exit_position(self, trade: TradeJournal, exit_reason: str, 
        ...
        if self.alert_service and result["success"]:
            try:
                msg = f"""
...
"""
        ...
        self.logger.info(
            f"Trade {trade.strategy_id} closed: P&L=₹{result['realized_pnl']:.2f}, Reason={exit_reason}"
        )
        ...
        return {
            ...
            "participant_positions": {
                ...
                "participants": {
                    "FII": { 
                        ...
                        "call_net": ext.fii_data['FII'].opt_net if ext.fii_data else 0,
                        "put_net": 0 
                    },
            ...
            "professional_recommendation": {
                "primary": {
                    ...
                    "strategy": analytics['next_weekly_mandate'].suggested_structure,
                    "capital_deploy_formatted": f"₹{analytics['next_weekly_mandate'].deployment_amount:,.0f}"
                }
            }
        }
...
def get_live_positions(
    ...
    try:
        ...
        # Fetch live prices from WebSocket cache
...
def get_correlation_report(
    ...
    correlation_manager = CorrelationManager(SessionLocal)
    report = correlation_manager.get_correlation_report()
