...
class VolGuardSystem:
    ...
    def run_complete_analysis(self) -> Dict:
        try:
            ...
            # Find the primary mandate for final recommendation
...
def get_fii_summary(token: str = Depends(verify_token)):
    """Lightweight endpoint for HUD top bar - FII summary"""
...
def get_fill_quality(token: str = Depends(verify_token)):
    """Expose fill quality metrics for UI chart"""
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
...
def get_pnl_attribution(db: Session = Depends(get_db), token: str = Depends(verify_token)):
    """P&L attribution for active positions"""
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
...
def list_gtts(db: Session = Depends(get_db), token: str = Depends(verify_token)):
    ...
    for trade in active_trades:
        if trade.gtt_order_ids:
            gtt_ids = json.loads(trade.gtt_order_ids)
            for gtt_id in gtt_ids:
                # Get instrument info from trade legs
                legs_data = json.loads(trade.legs_data)
...
async def websocket_subscribe(websocket: WebSocket, token: str = Depends(verify_token)):
    """WebSocket endpoint for subscribing to multiple instruments with JSON actions"""
    await volguard_system.ws_manager.connect(websocket)
    
    # Track subscribed instruments for this connection
    subscribed = set()
    ...
    try:
        while True:
            ...
            if data.get("action") == "subscribe":
                ...
                # Only unsubscribe from instruments we're subscribed to
                ...
                # Only change mode for instruments we're subscribed to
...
async def websocket_market_data(websocket: WebSocket, instrument_key: str, token: str = Depends(verify_token)):
    """Legacy WebSocket endpoint for single instrument - kept for backward compatibility"""
    await volguard_system.ws_manager.connect(websocket)
    try:
...
# ============================================================================
# API ENDPOINTS (EXISTING - UPDATED WITH PROFESSIONAL RECOMMENDATION)
# ============================================================================
...
def get_professional_dashboard(
    ...
    """Full professional dashboard with all metrics"""
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
    ...
    try:
        ...
        # Format participant positions for frontend (nested structure)
...
def get_correlation_report(
    ...
    """Get correlation report for active positions"""
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
...
def get_expiry_status(
    ...
    """Get expiry status for all expiries"""
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
