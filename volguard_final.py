...
# ============================================================================
# AUTO TRADING ENGINE - THE MISSING BRIDGE (RESTORED)
# ============================================================================

class AutoTradingEngine:
    """
    The missing nervous system that connects analytics to execution.
    Automatically triggers trades when mandates are favorable.
    """
    ...
    def should_trade_expiry(self, expiry_type: str) -> bool:
        """Check if we already have an active trade for this expiry type"""
        with self.db_session_factory() as db:
    ...
    def execute_mandate(self, analysis_data: Dict, mandate: TradingMandate) -> bool:
        """Execute a trading mandate if conditions are met"""
    ...
    def evaluate_all_mandates(self, analysis_data: Dict) -> Dict[str, bool]:
        """Evaluate all three expiry types and execute if conditions met"""
        results = {}
        ...
        for mandate_key, mandate in mandates:
            if mandate and mandate.is_trade_allowed:
                ...
                if success and mandate_key == "weekly_mandate":
                    break
            else:
...
class EconomicCalendarEngine:
    ...
    def fetch_calendar(self, days_ahead: int = 14) -> List[EconomicEvent]:
        """Fetch economic calendar with multiple fallback sources"""
    ...
    def _fetch_from_alpha_vantage(self, days_ahead: int) -> List[EconomicEvent]:
        """Fetch from Alpha Vantage API (paid, reliable)"""
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Parse Alpha Vantage format (implementation depends on their actual response structure)
    ...
    def _fetch_from_eodhd(self, days_ahead: int) -> List[EconomicEvent]:
        """Fetch from EOD Historical Data API (paid, reliable)"""
    ...
    def _fetch_from_investing_com(self, days_ahead: int) -> List[EconomicEvent]:
        """Fetch from Investing.com API (unofficial, fallback)"""
        events = []
...
class UpstoxFetcher:
    ...
    def _check_rate_limit(self, max_requests: int = 50, window_seconds: int = 1):
        """Rate limiter with proper thread safety using Condition"""
    ...
    def is_trading_day(self) -> bool:
        try:
            self._check_rate_limit()
            response = self.market_api.get_market_status(exchange="NSE", api_version="2.0")
...
class UpstoxOrderExecutor:
    ...
    def _get_fill_prices(self, order_ids: List[str]) -> Dict[str, float]:
        """Fetch actual fill prices from order history"""
        fill_prices = {}
                ...
                if response.status == "success" and response.data:
                    ...
                    if gtt_id:
                        ...
                        self.logger.info(
                            ...
                            f"Entry: ₹{leg.entry_price}, SL: ₹{stop_price}, Target: ₹{target_price}, Qty: {actual_qty}"
                        )
                else:
...
class AnalyticsEngine:
    ...
    def get_struct_metrics(self, chain: pd.DataFrame, spot: float, lot_size: int) -> StructMetrics:
        ...
        if abs(gex_ratio_pct) < DynamicConfig.get("GEX_STICKY_RATIO"):
            gex_regime = "STICKY"
        elif gex_ratio > 0:
        ...
        if skew_25d > DynamicConfig.get("SKEW_CRASH_FEAR"):
            skew_regime = "CRASH_FEAR"
        elif skew_25d < DynamicConfig.get("SKEW_MELT_UP"):
            skew_regime = "MELT_UP"
        else:
            skew_regime = "NORMAL"
...
class PositionMonitor:
    ...
    async def check_single_position(self, trade: TradeJournal, current_prices: Dict, db: Session, loop):
        ...
        if unrealized_pnl < -trade.max_loss * DynamicConfig.get("STOP_LOSS_MULTIPLIER"):
        ...
        elif unrealized_pnl > trade.max_profit * DynamicConfig.get("PROFIT_TARGET_MULTIPLIER"):
        ...
        return {
            ...
            "structure_analysis": {
                "weekly": {
                    ...
                    "gex_ratio_threshold": DynamicConfig.get("GEX_STICKY_RATIO"),
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_weekly'].gex_ratio) < DynamicConfig.get("GEX_STICKY_RATIO") else "STICKY",
                ...
                "next_weekly": {
                    ...
                    "gex_ratio_threshold": DynamicConfig.get("GEX_STICKY_RATIO"),
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_next_weekly'].gex_ratio) < DynamicConfig.get("GEX_STICKY_RATIO") else "STICKY",
                ...
                "monthly": {
                    ...
                    "gex_ratio_threshold": DynamicConfig.get("GEX_STICKY_RATIO"),
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_monthly'].gex_ratio) < DynamicConfig.get("GEX_STICKY_RATIO") else "STICKY",
...
if __name__ == "__main__":
    ...
    print(f"📏 Lot Size:        ✅ Dynamic from Upstox")
