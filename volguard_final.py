...
class UpstoxFetcher:
    def __init__(self, token: str):
        ...
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("✅ UpstoxFetcher initialized - V3.3 Full SDK + WebSocket + Smart Fallback")
    ...
    # ========================================================================
    # SMART FALLBACK METHODS (Use these for frontend)
    # ========================================================================
    ...
    def get_ltp_with_fallback(self, instrument_key: str) -> Optional[float]:
        """
        Get LTP with smart fallback:
        - Market open: WebSocket first, REST fallback
        - Market closed: REST API only
        """
    ...
    def get_bulk_ltp_with_fallback(self, instrument_keys: List[str]) -> Dict[str, Optional[float]]:
        """Get bulk LTP with smart fallback"""
    ...
    def get_ohlc_with_fallback(self, instrument_key: str, interval: str = "1d") -> Optional[Dict]:
        """Get OHLC data - always from REST (works 24/7)"""
    ...
    def get_full_quote_with_fallback(self, instrument_key: str) -> Optional[Dict]:
        """Get full market quote - always from REST (works 24/7)"""
    ...
    def get_ltp(self, instrument_key: str) -> Optional[float]:
        """Direct WebSocket access (for internal use)"""
...
class AnalyticsScheduler:
    ...
    async def start(self):
        ...
        # Wait for first price data (up to 30 seconds)
...
class PositionMonitor:
    ...
    async def check_all_positions(self):
        ...
        try:
            ...
            # Use smart fallback for prices
            current_prices = self.fetcher.get_bulk_ltp_with_fallback(list(set(all_instruments)))
    ...
    async def trigger_circuit_breaker(self, db: Session):
        ...
        for trade in active_trades:
            ...
            for instrument in instrument_tokens:
                price = self.fetcher.get_ltp_with_fallback(instrument)
                if price:
...
class VolGuardSystem:
    def __init__(self):
        ...
        self.logger.info("VolGuard System v3.3 initialized - FULL SDK + SMART FALLBACK + CORRELATION")
    ...
    try:
        ...
        return {
            ...
            "participant_positions": {
                "fii_direction": ext.fii_sentiment if ext.fii_data else "No data",
...
def health_check(db: Session = Depends(get_db)):
    ...
    # Fix the datetime subtraction issue
    analytics_cache_age = "N/A"
    if volguard_system and volguard_system.analytics_cache._last_calc_time:
        try:
            # Get current time with timezone (make it aware)
            now = datetime.now(pytz.UTC)
            
            # Get cache time (should be aware from AnalyticsCache)
            cache_time = volguard_system.analytics_cache._last_calc_time
            
            # If cache_time is naive for some reason, make it aware
