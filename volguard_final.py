...
class DynamicConfig:
    ...
    DEFAULTS = {
        ...
        "MAX_POSITION_RISK_PCT": 2.0,  # NEW: Max loss per trade as % of capital
        
        # Volatility Thresholds (Analytics Only)
        "HIGH_VOL_IVP": 75.0,
        "LOW_VOL_IVP": 25.0,
        ...
        # GEX & Structure (Analytics Only)
        "GEX_STICKY_RATIO": 0.03,
        "SKEW_CRASH_FEAR": 5.0,
        "SKEW_MELT_UP": -2.0,
        
        # FII Conviction (Analytics Only)
...
class SystemConfig:
    ...
    @property
    def MAX_POSITION_RISK_PCT(cls):
        return DynamicConfig.MAX_POSITION_RISK_PCT
...
class ConstructedStrategy:
    ...
    max_risk_amount: float  # NEW: Maximum loss in rupees
...
# ============================================================================
# JSON CACHE MANAGER - FIXED: Async HTTP calls in executor
# ============================================================================
...
class JSONCacheManager:
    ...
    def fetch_and_cache(self, force: bool = False) -> bool:
        """Synchronous version - called from executor"""
        with self._lock:
    ...
    async def schedule_daily_fetch(self):
        """Background task - now runs fetch in executor"""
        while True:
            try:
                now = datetime.now(self.ist_tz)
