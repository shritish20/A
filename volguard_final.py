...
from sqlalchemy import create_engine, Column, Integer, Float, String, DateTime, Boolean, JSON, Text, desc, event, text
...
class DynamicConfig:
    ...
    @classmethod
    @property
    def GTT_STOP_LOSS_MULTIPLIER(cls):
        return cls._values.get("GTT_STOP_LOSS_MULTIPLIER", 2.0)
    
    @classmethod
    @property
    def GTT_PROFIT_TARGET_MULTIPLIER(cls):
        return cls._values.get("GTT_PROFIT_TARGET_MULTIPLIER", 0.30)
    
    @classmethod
    @property
    def GTT_TRAILING_GAP(cls):
        return cls._values.get("GTT_TRAILING_GAP", 0.1)
    
    # Order timeouts
    @classmethod
    @property
    def ORDER_FILL_TIMEOUT_SECONDS(cls):
        return cls._values.get("ORDER_FILL_TIMEOUT_SECONDS", 15)
    
    @classmethod
    @property
    def ORDER_FILL_CHECK_INTERVAL(cls):
        return cls._values.get("ORDER_FILL_CHECK_INTERVAL", 0.5)
...
class SystemConfig:
    """Legacy static config - redirects to DynamicConfig"""
    ...
    @classmethod
    @property
    def BASE_CAPITAL(cls): return DynamicConfig.BASE_CAPITAL
    @classmethod
    @property
    def MAX_LOSS_PCT(cls): return DynamicConfig.MAX_LOSS_PCT
    @classmethod
    @property
    def PROFIT_TARGET(cls): return DynamicConfig.PROFIT_TARGET
    @classmethod
    @property
    def ENABLE_AUTO_TRADING(cls): return DynamicConfig.AUTO_TRADING
    @classmethod
    @property
    def ENABLE_MOCK_TRADING(cls): return DynamicConfig.ENABLE_MOCK_TRADING
    @classmethod
    @property
    def MIN_OI(cls): return DynamicConfig.MIN_OI
    @classmethod
    @property
    def MAX_BID_ASK_SPREAD_PCT(cls): return DynamicConfig.MAX_BID_ASK_SPREAD_PCT
    @classmethod
    @property
    def MAX_POSITION_RISK_PCT(cls): return DynamicConfig.MAX_POSITION_RISK_PCT
    @classmethod
    @property
    def STOP_LOSS_MULTIPLIER(cls): return DynamicConfig.STOP_LOSS_MULTIPLIER
    @classmethod
    @property
    def PROFIT_TARGET_MULTIPLIER(cls): return DynamicConfig.PROFIT_TARGET_MULTIPLIER
    @classmethod
    @property
    def WEEKLY_ALLOCATION_PCT(cls): return DynamicConfig.WEEKLY_ALLOCATION_PCT
    @classmethod
    @property
    def MONTHLY_ALLOCATION_PCT(cls): return DynamicConfig.MONTHLY_ALLOCATION_PCT
    @classmethod
    @property
    def NEXT_WEEKLY_ALLOCATION_PCT(cls): return DynamicConfig.NEXT_WEEKLY_ALLOCATION_PCT
    @classmethod
    @property
    def MAX_CONCURRENT_SAME_STRATEGY(cls): return DynamicConfig.MAX_CONCURRENT_SAME_STRATEGY
    @classmethod
    @property
    def PNL_DISCREPANCY_THRESHOLD(cls): return DynamicConfig.PNL_DISCREPANCY_THRESHOLD
    # GTT properties
    @classmethod
    @property
    def GTT_STOP_LOSS_MULTIPLIER(cls): return DynamicConfig.GTT_STOP_LOSS_MULTIPLIER
    @classmethod
    @property
    def GTT_PROFIT_TARGET_MULTIPLIER(cls): return DynamicConfig.GTT_PROFIT_TARGET_MULTIPLIER
    @classmethod
    @property
    def GTT_TRAILING_GAP(cls): return DynamicConfig.GTT_TRAILING_GAP
...
async def verify_token(x_upstox_token: Optional[str] = Header(None, alias="X-Upstox-Token")):
    """Verify Upstox access token - falls back to env for development"""
...
class SmartDataFetcher:
    """
    Intelligently chooses between WebSocket (real-time) and REST API (fallback)
    based on market hours and data availability.
    """
    ...
    def is_market_open(self) -> bool:
        ...
        is_time = (9 <= now.hour < 15) or (now.hour == 15 and now.minute <= 30)
        is_weekday = now.weekday() < 5  # Monday = 0, Friday = 4
        return is_time and is_weekday
    ...
    def get_ltp(self, instrument_key: str) -> Optional[float]:
        ...
        - Market open: Try WebSocket first, fallback to REST
        - Market closed: Use REST API (returns last traded price)
        """
        market_open = self.is_market_open()
        
        # During market hours, try WebSocket first (real-time)
        ...
        # Fallback to REST API (works 24/7, returns last traded price)
        try:
            ...
            if response.status == 'success' and response.data:
                ...
                if response_key in response.data:
                    ...
                    if hasattr(item, 'last_price'):
                        ...
                        source = "REST API" if not market_open else "WebSocket (fallback)"
                        self.logger.debug(f"{source} LTP for {instrument_key}: {price}")
                        return price
        except Exception as e:
    ...
    def get_bulk_ltp(self, instrument_keys: List[str]) -> Dict[str, Optional[float]]:
        """Get LTP for multiple instruments with fallback"""
        result = {}
        market_open = self.is_market_open()
        
        # During market hours, try WebSocket first
        if market_open and self.fetcher.market_streamer.is_connected:
        ...
        if missing:
            try:
                ...
                # Split into chunks of 50 (Upstox limit)
    ...
    def get_ohlc(self, instrument_key: str, interval: str = "1d") -> Optional[Dict]:
        """Get OHLC data - REST API works 24/7"""
        try:
            self.fetcher._check_rate_limit()
    ...
    def get_full_quote(self, instrument_key: str) -> Optional[Dict]:
        """Get full market quote - REST API works 24/7"""
        try:
            self.fetcher._check_rate_limit()
            ...
            if response.status == 'success' and response.data:
                if instrument_key in response.data:
                    item = response.data[instrument_key]
                    return {
...
class JSONCacheManager:
    ...
    def get_external_metrics(self) -> ExternalMetrics:
        ...
        # FIXED: Handle both string and datetime objects for event dates
        events = []
        for e_dict in cache.get("economic_events", []):
            e_dict_copy = e_dict.copy()
            
            # Handle event_date - could be string or datetime
            ...
            # Handle suggested_square_off_time - could be string or datetime
...
class ParticipantDataFetcher:
    ...
    def _parse_nse_data(data: Dict) -> Tuple[Optional[Dict], Optional[Dict], float, str, bool]:
        """Parse NSE format participant data"""
        try:
    ...
    def _parse_fyers_data(data: Dict) -> Tuple[Optional[Dict], Optional[Dict], float, str, bool]:
        """Parse Fyers format participant data as fallback"""
        try:
...
# ============================================================================
# ECONOMIC CALENDAR ENGINE - FIXED
# ============================================================================
...
class EconomicCalendarEngine:
    ...
    def fetch_calendar(self, days_ahead: int = 14) -> List[EconomicEvent]:
        """Fetch economic calendar with caching and multiple fallback sources"""
        ...
        sources = [
            ...
            self._fetch_from_mock  # Last resort
        ]
        ...
        for source in sources:
            try:
                ...
                self.logger.warning(f"Calendar source failed: {e}")
                continue
    ...
    def _fetch_from_forexfactory(self, days_ahead: int) -> List[EconomicEvent]:
        """Fetch from ForexFactory as backup"""
        events = []
    ...
    def _fetch_from_mock(self, days_ahead: int) -> List[EconomicEvent]:
        ...
        for mock in mock_events:
            if mock["days_offset"] <= days_ahead:
                event_date = now_ist + timedelta(days=mock["days_offset"])
                event_date = event_date.replace(hour=10, minute=30)
                
                event_type = "VETO" if mock["is_veto"] else "HIGH_IMPACT"
                
                square_off = self.get_square_off_for_event(event_date) if mock["is_veto"] else None
                
                event = EconomicEvent(
                    title=mock["title"],
                    country=mock["country"],
                    event_date=event_date,
                    impact_level=event_type,
                    event_type=event_type,
                    forecast="-",
                    previous="-",
                    days_until=mock["days_offset"],
                    hours_until=mock["days_offset"] * 24,
                    is_veto_event=mock["is_veto"],
                    suggested_square_off_time=square_off
                )
                events.append(event)
...
class VolGuardMarketStreamer:
    ...
    def _check_ws_rate_limit(self):
        """Rate limit WebSocket operations (50/second)"""
    ...
    def connect(self, instrument_keys: Optional[List[str]] = None, mode: str = "ltpc"):
        try:
            ...
            # More aggressive reconnect settings
    ...
    def get_ltp(self, instrument_key: str) -> Optional[float]:
        """Get LTP for a single instrument from cache"""
        with self._lock:
            return self._latest_prices.get(instrument_key)
    
    def get_bulk_ltp(self, instrument_keys: List[str]) -> Dict[str, Optional[float]]:
        """Get LTP for multiple instruments from cache"""
        with self._lock:
...
class VolGuardPortfolioStreamer:
    ...
    def get_order_status(self, order_id: str) -> Optional[Dict]:
        """Get order status with partial fill tracking"""
        with self._lock:
    ...
    def _fetch_order_status_sync(self, order_id: str) -> Optional[Dict]:
        """Synchronous REST fallback for order status"""
        try:
    ...
    async def _fetch_order_status_async(self, order_id: str) -> Optional[Dict]:
        """Async wrapper for sync REST call"""
        loop = asyncio.get_event_loop()
...
# ============================================================================
# UPSTOX FETCHER - WITH CONNECTION POOLING FIX
# ============================================================================
...
class UpstoxFetcher:
    def __init__(self, token: str):
        ...
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("✅ UpstoxFetcher initialized - V3.5 Full SDK + WebSocket + Smart Fallback")
    ...
    def get_lot_size_for_expiry(self, expiry_date: date) -> int:
        ...
        default_lot_size = 50
        self.logger.warning(f"⚠️ Using default lot size {default_lot_size} for {expiry_str}")
...
class UpstoxOrderExecutor:
    ...
    def _wait_for_fills(self, order_ids: List[str], expected_quantities: Dict[str, int] = None) -> Dict[str, Dict]:
        ...
        timeout = DynamicConfig.ORDER_FILL_TIMEOUT_SECONDS
        check_interval = DynamicConfig.ORDER_FILL_CHECK_INTERVAL
                ...
                if status:
                    if status.get('mapped_status') in ['FILLED', 'PARTIAL']:
                        ...
                        if status.get('mapped_status') == 'FILLED':
                            ...
                            self.logger.info(f"⚠️ Order {oid} partially filled: {filled_qty} units")
        ...
        if unfilled:
            self.logger.warning(f"⚠️ Orders not filled within timeout: {unfilled}")
    ...
    def _place_gtt_stop_losses(self, strategy: ConstructedStrategy, filled_orders: Dict[str, Dict]) -> List[str]:
        ...
        for leg in strategy.legs:
            ...
            stop_price = round(leg.entry_price * DynamicConfig.GTT_STOP_LOSS_MULTIPLIER, 2)
            target_price = round(leg.entry_price * DynamicConfig.GTT_PROFIT_TARGET_MULTIPLIER, 2)
            trailing_gap = DynamicConfig.GTT_TRAILING_GAP
            
            actual_qty = leg.quantity
            for oid, fill_info in filled_orders.items():
                if fill_info.get('filled_quantity', 0) > 0:
                    actual_qty = fill_info['filled_quantity']
            ...
            try:
                ...
                body = upstox_client.GttPlaceOrderRequest(
                    ...
                    rules=[target_rule, stoploss_rule]
                )
                ...
                if response.status == "success" and response.data:
                    ...
                    if gtt_id:
                        ...
                        self.logger.info(
                            ...
                            f"SL: ₹{stop_price}, Target: ₹{target_price}, Qty: {actual_qty}"
                        )
                else:
            ...
            if response.status in ["success", "partial_success"]:
                ...
                for i, leg in enumerate(ordered_legs):
                    ...
                    if correlation_id in correlation_map:
                        order_id = correlation_map[correlation_id]
...
class AnalyticsEngine:
    ...
    def get_struct_metrics(self, chain: pd.DataFrame, spot: float, lot_size: int) -> StructMetrics:
        ...
        # FIXED: GEX ratio should be a small decimal (0.0018), not 1801%
        gex_ratio = round(net_gex / total_oi_value, 6) if total_oi_value > 0 else 0
        gex_ratio_pct = round(gex_ratio * 100, 4) if total_oi_value > 0 else 0
        gex_weighted = round(net_gex / 1_000_000, 2)
        
        # Use the percentage for threshold comparison (0.03% = 0.0003 in decimal)
        if abs(gex_ratio_pct) < DynamicConfig.GEX_STICKY_RATIO:  # Compare with 0.03
            gex_regime = "STICKY"
        elif gex_ratio > 0:
        ...
        pcr_atm = round(atm_row.iloc[0]['pe_oi'] / atm_row.iloc[0]['ce_oi'], 2) if not atm_row.empty and atm_row.iloc[0]['ce_oi'] > 0 else 1.0
        ...
        return StructMetrics(
            net_gex=net_gex,
            gex_ratio=gex_ratio_pct,  # Store percentage for display
    ...
    def get_edge_metrics(self, weekly_chain: pd.DataFrame, monthly_chain: pd.DataFrame,
        ...
        def get_iv(chain):
            ...
            return round((atm_row['ce_iv'] + atm_row['pe_iv']) / 2, 2)
...
class RegimeEngine:
    ...
    def calculate_scores(self, vol_metrics: VolMetrics, struct_metrics: StructMetrics,
        ...
        total_score = round(vol_score + struct_score + edge_score + external_score, 2)
...
class PositionMonitor:
    ...
    async def start_monitoring(self):
        ...
        self.logger.info(f"Position monitoring started ({DynamicConfig.MONITOR_INTERVAL_SECONDS}s intervals - Smart Fallback)")
    ...
    async def check_single_position(self, trade: TradeJournal, current_prices: Dict, db: Session, loop):
        ...
        for leg in legs_data:
            ...
            leg_pnl = round((current_price - entry_price) * qty * multiplier, 2)
...
class VolGuardSystem:
    def __init__(self):
        ...
        self.logger.info("VolGuard System v3.5 initialized - FULL SDK + SMART FALLBACK + WEBSOCKET")
...
async def lifespan(app: FastAPI):
    logger.info("=" * 70)
    logger.info("VolGuard v3.5 Starting... (ALL UPSTOX SDK ISSUES FIXED + WEBSOCKET)")
    logger.info("=" * 70)
    ...
    logger.info(f"GTT Stop Loss Multiplier: {DynamicConfig.GTT_STOP_LOSS_MULTIPLIER}x")
    logger.info(f"GTT Profit Target Multiplier: {DynamicConfig.GTT_PROFIT_TARGET_MULTIPLIER}x")
    logger.info(f"GTT Trailing Gap: {DynamicConfig.GTT_TRAILING_GAP}")
    ...
    if SystemConfig.TELEGRAM_TOKEN and SystemConfig.TELEGRAM_CHAT_ID:
        ...
        alert_service.send(
            ...
            "System Online - All Upstox SDK Issues Fixed + WebSocket",
...
app = FastAPI(
    ...
    description="Professional Options Trading System - Full Upstox SDK Compliance + WebSocket",
    version="3.5.0",
    lifespan=lifespan
)
...
# ============================================================================
# WEBSOCKET ENDPOINTS FOR REACT FRONTEND
# ============================================================================
...
async def websocket_market_data(websocket: WebSocket, instrument_key: str, token: str = Depends(verify_token)):
    ...
    try:
        ...
        while True:
            # Get latest price from streamer
...
async def websocket_portfolio(websocket: WebSocket, token: str = Depends(verify_token)):
    ...
    try:
        while True:
            # Get latest positions from portfolio streamer
...
async def websocket_subscribe(websocket: WebSocket, token: str = Depends(verify_token)):
    """WebSocket endpoint for subscribing to multiple instruments"""
    await volguard_system.ws_manager.connect(websocket)
    try:
        while True:
            ...
            if data.get("action") == "subscribe":
                ...
                success = volguard_system.fetcher.subscribe_market_data(instruments, mode)
                await websocket.send_json({
                ...
                instruments = data.get("instruments", [])
                success = volguard_system.fetcher.unsubscribe_market_data(instruments)
                await websocket.send_json({
                    "type": "subscription_result",
                    "action": "unsubscribe",
                    "instruments": instruments,
...
# ============================================================================
# API ENDPOINTS (Preserved from your original)
# ============================================================================
...
def get_professional_dashboard(
    ...
    try:
        ...
        # Add drivers based on metrics
        if vol.vov_zscore > 2:
            score_drivers["weekly"]["negative"].append({
                "factor": f"High VOV ({vol.vov_zscore}σ)",
                "impact": "-3.0"
            })
            
            # Same drivers apply to all expiries
            score_drivers["next_weekly"]["negative"].append({
                "factor": f"High VOV ({vol.vov_zscore}σ)",
                "impact": "-3.0"
            })
            score_drivers["monthly"]["negative"].append({
                "factor": f"High VOV ({vol.vov_zscore}σ)",
                "impact": "-3.0"
            })
        
        if vol.ivp_1yr > 30 and vol.ivp_1yr < 70:
            score_drivers["weekly"]["positive"].append({
                "factor": f"Fair IVP ({vol.ivp_1yr}%)",
                "impact": "+1.0"
            })
            score_drivers["next_weekly"]["positive"].append({
                "factor": f"Fair IVP ({vol.ivp_1yr}%)",
                "impact": "+1.0"
            })
            score_drivers["monthly"]["positive"].append({
                "factor": f"Fair IVP ({vol.ivp_1yr}%)",
                "impact": "+1.0"
            })
        ...
        # Build rationale
        rationale = {
            "weekly": ["Regime Unfavorable: Preserve capital"] if not analytics['weekly_mandate'].is_trade_allowed else [],
            "next_weekly": ["Regime Unfavorable: Preserve capital"] if not analytics['next_weekly_mandate'].is_trade_allowed else [],
            "monthly": ["Regime Unfavorable: Preserve capital"] if not analytics['monthly_mandate'].is_trade_allowed else []
        }
        ...
        return {
            ...
            "structure_analysis": {
                "weekly": {
                    ...
                    "gex_ratio": analytics['struct_weekly'].gex_ratio,
                    "gex_ratio_formatted": f"{analytics['struct_weekly'].gex_ratio:.4f}%",
                    "gex_ratio_threshold": DynamicConfig.GEX_STICKY_RATIO,
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_weekly'].gex_ratio) < DynamicConfig.GEX_STICKY_RATIO else "STICKY",
                    "pcr_all": analytics['struct_weekly'].pcr,
                    "pcr_regime": analytics['struct_weekly'].oi_regime,
                    "max_pain": analytics['struct_weekly'].max_pain,
                    "skew_25d": analytics['struct_weekly'].skew_25d,
                    "skew_25d_formatted": f"{analytics['struct_weekly'].skew_25d:+.2f}%",
                ...
                "next_weekly": {
                    ...
                    "gex_ratio": analytics['struct_next_weekly'].gex_ratio,
                    "gex_ratio_formatted": f"{analytics['struct_next_weekly'].gex_ratio:.4f}%",
                    "gex_ratio_threshold": DynamicConfig.GEX_STICKY_RATIO,
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_next_weekly'].gex_ratio) < DynamicConfig.GEX_STICKY_RATIO else "STICKY",
                    "pcr_all": analytics['struct_next_weekly'].pcr,
                    "pcr_regime": analytics['struct_next_weekly'].oi_regime,
                    "max_pain": analytics['struct_next_weekly'].max_pain,
                    "skew_25d": analytics['struct_next_weekly'].skew_25d,
                    "skew_25d_formatted": f"{analytics['struct_next_weekly'].skew_25d:+.2f}%",
                ...
                "monthly": {
                    ...
                    "gex_ratio": analytics['struct_monthly'].gex_ratio,
                    "gex_ratio_formatted": f"{analytics['struct_monthly'].gex_ratio:.4f}%",
                    "gex_ratio_threshold": DynamicConfig.GEX_STICKY_RATIO,
                    "gex_ratio_regime": "SLIPPERY" if abs(analytics['struct_monthly'].gex_ratio) < DynamicConfig.GEX_STICKY_RATIO else "STICKY",
                    "pcr_all": analytics['struct_monthly'].pcr,
                    "pcr_regime": analytics['struct_monthly'].oi_regime,
                    "max_pain": analytics['struct_monthly'].max_pain,
                    "skew_25d": analytics['struct_monthly'].skew_25d,
                    "skew_25d_formatted": f"{analytics['struct_monthly'].skew_25d:+.2f}%",
            ...
            "regime_scores": {
                "weekly": {
                    "composite": {
                        ...
                        "score_out_of_10": round(analytics['weekly_score'].total_score, 2),
                    ...
                    "contributions": {
                        "volatility": round(analytics['weekly_score'].vol_score * 0.5, 2),
                        "structure": round(analytics['weekly_score'].struct_score * 0.25, 2),
                        "edge": round(analytics['weekly_score'].edge_score * 0.25, 2),
                        "total": round(analytics['weekly_score'].total_score, 2)
                    },
                    "components": {
                        "volatility": {
                            "score": round(analytics['weekly_score'].vol_score, 2),
                            "score_out_of_10": round(analytics['weekly_score'].vol_score, 2)
                        },
                        "structure": {
                            "score": round(analytics['weekly_score'].struct_score, 2),
                            "score_out_of_10": round(analytics['weekly_score'].struct_score, 2)
                        },
                        "edge": {
                            "score": round(analytics['weekly_score'].edge_score, 2),
                            "score_out_of_10": round(analytics['weekly_score'].edge_score, 2)
                        }
                    }
                },
                "next_weekly": {
                    "composite": {
                        ...
                        "score_out_of_10": round(analytics['next_weekly_score'].total_score, 2),
                    ...
                    "contributions": {
                        "volatility": round(analytics['next_weekly_score'].vol_score * 0.5, 2),
                        "structure": round(analytics['next_weekly_score'].struct_score * 0.25, 2),
                        "edge": round(analytics['next_weekly_score'].edge_score * 0.25, 2),
                        "total": round(analytics['next_weekly_score'].total_score, 2)
                    },
                    "components": {
                        "volatility": {
                            "score": round(analytics['next_weekly_score'].vol_score, 2),
                            "score_out_of_10": round(analytics['next_weekly_score'].vol_score, 2)
                        },
                        "structure": {
                            "score": round(analytics['next_weekly_score'].struct_score, 2),
                            "score_out_of_10": round(analytics['next_weekly_score'].struct_score, 2)
                        },
                        "edge": {
                            "score": round(analytics['next_weekly_score'].edge_score, 2),
                            "score_out_of_10": round(analytics['next_weekly_score'].edge_score, 2)
                        }
                    }
                },
                "monthly": {
                    "composite": {
                        ...
                        "score_out_of_10": round(analytics['monthly_score'].total_score, 2),
                    ...
                    "contributions": {
                        "volatility": round(analytics['monthly_score'].vol_score * 0.5, 2),
                        "structure": round(analytics['monthly_score'].struct_score * 0.25, 2),
                        "edge": round(analytics['monthly_score'].edge_score * 0.25, 2),
                        "total": round(analytics['monthly_score'].total_score, 2)
                    },
                    "components": {
                        "volatility": {
                            "score": round(analytics['monthly_score'].vol_score, 2),
                            "score_out_of_10": round(analytics['monthly_score'].vol_score, 2)
                        },
                        "structure": {
                            "score": round(analytics['monthly_score'].struct_score, 2),
                            "score_out_of_10": round(analytics['monthly_score'].struct_score, 2)
                        },
                        "edge": {
                            "score": round(analytics['monthly_score'].edge_score, 2),
                            "score_out_of_10": round(analytics['monthly_score'].edge_score, 2)
                        }
                    }
                }
            },
            ...
            "mandates": {
                ...
                "next_weekly": {
                    ...
                    "rationale": rationale["next_weekly"],
                    "capital": {
...
def root():
    return {
        ...
        "gtt_config": {
            "stop_loss_multiplier": DynamicConfig.GTT_STOP_LOSS_MULTIPLIER,
            "profit_target_multiplier": DynamicConfig.GTT_PROFIT_TARGET_MULTIPLIER,
            "trailing_gap": DynamicConfig.GTT_TRAILING_GAP
        },
        "websocket": {
...
def health_check(db: Session = Depends(get_db)):
    ...
    if volguard_system and volguard_system.analytics_cache._last_calc_time:
        try:
            ...
            analytics_cache_age = age_seconds // 60
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("🚀 VolGuard v3.5 - PRODUCTION READY - ALL ISSUES FIXED + WEBSOCKET")
    print("=" * 80)
    ...
    print(f"   • Stop Loss:     {DynamicConfig.DEFAULTS['GTT_STOP_LOSS_MULTIPLIER']}x")
    print(f"   • Profit Target: {DynamicConfig.DEFAULTS['GTT_PROFIT_TARGET_MULTIPLIER']}x")
    print(f"   • Trailing Gap:  {DynamicConfig.DEFAULTS['GTT_TRAILING_GAP']}")
    ...
    print(f"🔌 WebSocket:       ✅ get_bulk_ltp added (FIXED)")
    print(f"🌐 WebSocket APIs:  ✅ /api/ws/* endpoints for React")
    print(f"📊 Numbers:         ✅ ALL rounded properly (no more 66.66666)")
    print("=" * 80)
    print(f"📚 API Documentation: http://localhost:{SystemConfig.PORT}/docs")
    print(f"🔌 WebSocket Test:   ws://localhost:{SystemConfig.PORT}/api/ws/market/NSE_INDEX|Nifty%2050")
    print("=" * 80)
