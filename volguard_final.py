...
class EconomicCalendarEngine:
    ...
    def fetch_calendar(self, days_ahead: int = 14) -> List[EconomicEvent]:
        ...
        if not events:
            self.logger.error("No calendar events fetched")
            if self.alert_service:
                self.alert_service.send(
                    "Calendar API Failure",
                    "Economic calendar APIs failed - no veto events detected",
                    AlertPriority.HIGH,
...
class UpstoxOrderExecutor:
    ...
    def exit_position(self, trade: TradeJournal, exit_reason: str, 
        ...
        for leg in legs_data:
            instrument_key = leg['instrument_token']
            if instrument_key in current_prices and current_prices[instrument_key] is not None:
                exit_price = current_prices[instrument_key]
            else:
                exit_price = leg['entry_price']
                pnl_approximate = True
                self.logger.warning(f"Missing exit price for {instrument_key} - using entry price")
        ...
        self.logger.info(
            f"Trade {trade.strategy_id} closed: P&L=₹{realized_pnl:.2f}{' (approximate)' if pnl_approximate else ''}, Reason={exit_reason}"
        )
...
async def lifespan(app: FastAPI):
    ...
    # Set alert service in calendar engine
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("🚀 VolGuard v3.5 - PRODUCTION READY")
    print("=" * 80)
